...
Start with `AGENTS.md` and `docs/code-organization.md`, then search the
implementation and callers. Kit owns reusable mechanisms; the Ultima
Underworld ruleset owns policy and meaning; the Host owns lifecycle
composition; the DOM only displays facts. Customize those owner pointers as
the product grows.
